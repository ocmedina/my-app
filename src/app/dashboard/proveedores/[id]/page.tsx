...
export default async function SupplierDetailPage({ params }: { params: { id: string } }) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <Link href="/dashboard/proveedores" className="text-blue-600 mt-4 inline-block">
  ...
  return (
    <div className="space-y-6">
      ...
      <RegisterSupplierPayment supplierId={supplier.id} currentDebt={supplier.debt || 0} />
          ...
          {history.length === 0 ? (
            ...
            history.map(item => {
              ...
              const displayTitle = isCompra 
                ? `Factura #${'invoice_number' in item && item.invoice_number ? item.invoice_number : item.id.substring(0, 8)}`
                : 'Pago Realizado';
              
              return (
                <div key={item.id} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 p-4 border rounded-lg hover:bg-gray-50 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className={`p-3 rounded-full flex-shrink-0 ${isCompra ? 'bg-red-100' : 'bg-green-100'}`}>
                      {isCompra ? (
                  ...
                  <p className={`font-bold text-xl ${isCompra ? 'text-red-600' : 'text-green-600'} sm:text-right`}>
