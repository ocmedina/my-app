  ...
  const handleFinalizeSale = async () => {
    ...
    if (paid > total) {
      const confirmOverpay = window.confirm(
        `El monto pagado ($${paid.toFixed(2)}) es mayor al total ($${total.toFixed(2)}). ¿Deseas continuar?`
      )
    ...
    try {
      ...
      toast.error(`Error al registrar la venta: ${error.message || 'Error desconocido'}`)
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              >
                {customers.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.full_name} {c.customer_type === 'mayorista' ? '(Mayorista)' : '(Minorista)'}
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-xl font-bold mb-4 text-gray-800">Carrito y Pago</h2>
