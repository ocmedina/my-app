...
import { 
  ...
  FaCheck, FaCheckCircle, FaClock, FaPrint, FaSpinner 
} from 'react-icons/fa'
import { PDFDownloadLink } from '@react-pdf/renderer'
import OrderPDFDocument from '@/components/OrderPDFDocument' // Asume que este componente ya existe
...
// --- Modal de Confirmación de Entrega (CON FIADO) ---
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full">
        <div className="bg-gradient-to-r from-green-600 to-emerald-600 px-6 py-5 rounded-t-3xl flex justify-between items-center">
          <h2 className="text-xl font-bold text-white flex items-center gap-2"><FaTruck /> Confirmar Entrega y Pago</h2>
          <button onClick={onClose} disabled={isDelivering} className="text-white hover:bg-white hover:bg-opacity-20 rounded-full p-2 transition-all"><FaTimes size={20} /></button>
        </div>
        <div className="p-8 space-y-5">
          ...
          <div>
            ...
            <select value={paymentMethod} onChange={e => setPaymentMethod(e.target.value)} className="mt-1 w-full p-3 border-2 border-gray-200 rounded-xl bg-gray-50">
          ...
          <button onClick={handleConfirm} disabled={isDelivering} className="w-full py-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white font-bold rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all shadow-lg flex items-center justify-center gap-2 text-lg disabled:opacity-50 disabled:cursor-not-allowed">
            {isDelivering ? (
              <> <div className="animate-spin rounded-full h-5 w-5 border-3 border-white border-t-transparent"></div> Marcando... </>
            ) : (
              <> <FaCheck className="text-xl" /> Marcar como Entregado </>
            )}
          </button>
          <button onClick={onClose} disabled={isDelivering} className="w-full mt-3 py-3 text-gray-600 font-semibold hover:bg-gray-100 rounded-xl transition-all disabled:opacity-50">
            Cancelar
          </button>
        </div>
      </div>
...
// --- Modal de REMITO PDF ---
function RemitoModal({ isOpen, onClose, orderId }: { isOpen: boolean, onClose: () => void, orderId: string | null }) {
  ...
  useEffect(() => {
    if (isOpen && orderId) {
      ...
      const fetchFullOrder = async () => {
        const { data: order, error } = await supabase.from('orders').select('*, customers(*), order_items(*, products(*))').eq('id', orderId).single();
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full">
        <div className="bg-gradient-to-r from-gray-700 to-gray-900 px-6 py-5 rounded-t-3xl flex justify-between items-center">
          <h2 className="text-xl font-bold text-white flex items-center gap-2"><FaPrint /> Generar Remito</h2>
          <button onClick={onClose} className="text-white hover:bg-white hover:bg-opacity-20 rounded-full p-2"><FaTimes size={20} /></button>
        </div>
        <div className="p-8 text-center">
          {loading || !orderData ? (
            ...
            <>
              <p className="mb-6 text-gray-700">El remito para <span className="font-bold">{orderData.customers?.full_name}</span> está listo.</p>
              <PDFDownloadLink
              ...
              <button onClick={onClose} className="w-full mt-3 py-3 text-gray-600 font-semibold hover:bg-gray-100 rounded-xl">Cerrar</button>
            </>
          )}
        </div>
      </div>
...
export default function RepartoPage() {
  ...
  useEffect(() => {
    ...
    const ordersSubscription = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'orders', filter: `profile_id=eq.${currentUser.id}`},
        (payload) => {
          // Ya no dependemos solo de la suscripción, sino del refresco manual
  ...
  // --- LÓGICA DE CÁLCULO DE TOTAL Y SUBTOTAL (CORREGIDA) ---
  ...
  const handleFinalizeOrder = async () => {
    ...
    for (const item of cart) {
      // Usamos el stock que ya tenemos en el item del carrito
      if (item.quantity > (item.stock || 0)) {
        toast.error(`Stock insuficiente para "${item.name}". Solo quedan ${item.stock}.`);
        setLoading(false);
    ...
    // 👇 --- CORRECCIÓN AQUÍ --- 👇
    if (currentUser) {
        fetchDailyHistory(currentUser.id); // Llama al refresco manual
    }
    setCart([]);
  ...
  const handleOpenRemitoModal = (orderId: string) => {
  ...
  const handleConfirmDelivery = async (orderId: string, amountPaid: number, paymentMethod: string) => {
    ...
    if (debtGenerated > 0) {
      const { data: customerData } = await supabase.from('customers').select('debt').eq('id', order.customer_id).single();
      const currentDebt = (customerData?.debt as number) || 0;
      const newDebt = currentDebt + debtGenerated;
      await supabase.from('customers').update({ debt: newDebt }).eq('id', order.customer_id);
    }
    
    // 2. Registrar movimientos en la cuenta corriente
    const movements = [
      { customer_id: order.customer_id, order_id: order.id, type: 'compra', amount: total, comment: `Pedido ${order.id.substring(0,8)}` }
    ];
    if (amountPaid > 0) {
      movements.push(
        { customer_id: order.customer_id, order_id: order.id, type: 'pago', amount: amountPaid, comment: `Pago en entrega (${paymentMethod})` }
      );
    }
      ...
      <header className="bg-white shadow-md border-b border-gray-200 sticky top-0 z-40">
        <div className="px-4 py-3 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">FrontStock</h1>
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2">
              ...
              <span className="font-semibold text-sm text-gray-700 max-w-[120px] truncate">{currentUser?.email?.split('@')[0]}</span>
            </div>
            <button onClick={handleLogout} className="flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 font-medium rounded-lg transition-colors">
              <FaSignOutAlt /> 
        ...
        <div className="flex border-t border-gray-200">
          <button onClick={() => setView('new_order')} className={`flex-1 py-3 text-center font-semibold transition-all flex items-center justify-center gap-2 ${view === 'new_order' ? 'bg-blue-50 border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:bg-gray-50'}`}>
            <FaEdit className="text-lg"/> <span>Tomar Pedido</span>
          </button>
          <button onClick={() => setView('history')} className={`flex-1 py-3 text-center font-semibold transition-all flex items-center justify-center gap-2 relative ${view === 'history' ? 'bg-blue-50 border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:bg-gray-50'}`}>
            <FaClipboardList className="text-lg"/> <span>Mis Entregas</span>
            {pendingOrdersCount > 0 && (<span className="absolute top-1 right-2 sm:right-1/4 bg-red-500 text-white text-xs font-bold rounded-full w-5 h-5 flex items-center justify-center animate-pulse">{pendingOrdersCount}</span>)}
          </button>
        </div>
      </header>
        ...
        <main className="p-4 space-y-4 pb-32">
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
            <div className="bg-gradient-to-r from-blue-600 to-purple-600 px-5 py-3">
              <label className="flex items-center gap-2 text-sm font-semibold text-white"><FaUser /> Cliente Seleccionado</label>
            </div>
            <div className="p-5">
              <select value={selectedCustomer?.id || ''} onChange={(e) => setSelectedCustomer(customers.find(c => c.id === e.target.value) || null)} className="w-full p-4 border-2 border-gray-200 rounded-xl text-lg font-semibold bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all outline-none text-gray-800">
                {customers.map(c => <option key={c.id} value={c.id}>{c.full_name}</option>)}
              </select>
              {selectedCustomer && (
                <div className="mt-4 flex items-center gap-2">
                  <span className={`text-xs font-medium px-3 py-1.5 rounded-full ${selectedCustomer.customer_type === 'mayorista' ? 'bg-purple-100 text-purple-700' : 'bg-green-100 text-green-700'}`}>
          ...
          <div className="bg-white p-5 rounded-2xl shadow-lg border border-gray-200">
            <div className="flex justify-between items-center mb-4">
              <h2 className="font-bold text-gray-800 flex items-center gap-2"><FaShoppingCart className="text-blue-600" /> Productos del Pedido</h2>
              <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white text-sm font-semibold rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all shadow-md">
                <FaPlus /> Añadir
              </button>
            </div>
              ...
              {cart.length === 0 ? (
                ...
                cart.map(item => (
                  <div key={item.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-200">
                    ...
                    <div className="flex items-center gap-3">
                      <button onClick={() => handleUpdateQuantity(item.id, item.quantity - 1)} className="w-8 h-8 flex items-center justify-center bg-white border-2 border-red-200 text-red-600 rounded-lg hover:bg-red-50 transition-all font-bold"><FaMinus size={12} /></button>
                      <span className="font-bold text-lg w-8 text-center">{item.quantity}</span>
                      <button onClick={() => handleUpdateQuantity(item.id, item.quantity + 1)} className="w-8 h-8 flex items-center justify-center bg-white border-2 border-green-200 text-green-600 rounded-lg hover:bg-green-50 transition-all font-bold"><FaPlus size={12} /></button>
                    </div>
                  </div>
                ))
              )}
        ...
        <main className="p-4 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-gradient-to-br from-orange-500 to-red-500 rounded-xl p-4 text-white shadow-lg">
              <div className="flex items-center justify-between"><div><p className="text-sm opacity-90 font-medium">Pendientes</p><p className="text-3xl font-bold mt-1">{pendingOrdersCount}</p></div><FaClock className="text-4xl opacity-80" /></div>
            </div>
            <div className="bg-gradient-to-br from-green-500 to-emerald-500 rounded-xl p-4 text-white shadow-lg">
              <div className="flex items-center justify-between"><div><p className="text-sm opacity-90 font-medium">Entregados</p><p className="text-3xl font-bold mt-1">{deliveredOrdersCount}</p></div><FaCheckCircle className="text-4xl opacity-80" /></div>
            </div>
          </div>
          <div className="bg-white p-5 rounded-2xl shadow-lg border border-gray-200">
            <h2 className="font-bold text-gray-800 mb-4 flex items-center gap-2 text-lg"><FaClipboardList className="text-blue-600" /> Pedidos de Hoy ({dailyOrders.length})</h2>
              ...
              {dailyOrders.length > 0 ? dailyOrders.map(order => (
                <li key={order.id} className="p-4 border-2 border-gray-200 rounded-xl hover:border-blue-300 transition-all">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex-1"><p className="font-semibold text-gray-800">{order.customers.full_name}</p><p className="text-xs text-gray-500 mt-1">{new Date(order.created_at).toLocaleTimeString('es-AR', { hour: '2-digit', minute: '2-digit' })}</p></div>
                    <p className="font-bold text-xl text-green-600">${order.total_amount.toFixed(2)}</p>
                  </div>
                  <div className="flex items-center justify-between gap-2 mt-3 pt-3 border-t">
                    <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full ${order.status === 'pendiente' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>
                      {order.status === 'pendiente' ? <FaClock /> : <FaCheckCircle />}
                      {order.status === 'pendiente' ? 'Pendiente' : 'Entregado'}
                    </span>
                    <div className="flex gap-2">
                      <button onClick={() => handleOpenRemitoModal(order.id)} className="flex items-center gap-2 px-3 py-2 bg-gray-600 text-white text-xs font-semibold rounded-lg hover:bg-gray-700 transition-all">
                        <FaPrint /> Remito
                      </button>
                      {order.status === 'pendiente' && (
                        <button onClick={() => handleOpenDeliveryModal(order)} className="flex items-center gap-2 px-3 py-2 bg-green-600 text-white text-xs font-semibold rounded-lg hover:bg-green-700 transition-all">
                          <FaTruck /> Entregar
                        </button>
                      )}
                    </div>
        ...
        <footer className="fixed bottom-0 left-0 right-0 bg-white border-t-2 border-gray-200 p-4 shadow-2xl">
          <div className="max-w-lg mx-auto">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <input type="number" value={discount} onChange={e => setDiscount(Math.max(0, Math.min(100, parseFloat(e.target.value) || 0)))} className="w-20 p-1 border-2 border-gray-200 rounded-lg text-right font-semibold" placeholder="0"/>
            ...
            <button onClick={handleFinalizeOrder} disabled={loading || cart.length === 0 || !selectedCustomer} className="w-full py-4 bg-gradient-to-r from-green-600 to-green-700 text-white font-bold rounded-xl disabled:from-gray-400 disabled:to-gray-500 disabled:cursor-not-allowed text-lg shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-3 hover:from-green-700 hover:to-green-800">
              {loading ? ( <> <div className="animate-spin rounded-full h-5 w-5 border-3 border-white border-t-transparent"></div> Guardando... </> ) : ( <> <FaCheck className="text-xl" /> Guardar Pedido </> )}
            </button>
          </div>
        </footer>
      )}
