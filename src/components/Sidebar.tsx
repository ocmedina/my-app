...
const navLinks = [
  ...
  { href: '/dashboard/usuarios', label: 'Usuarios', icon: HiOutlineUserGroup, adminOnly: true }, // <-- 2. ENLACE AÑADIDO
]
...
export default function Sidebar() {
  ...
  const visibleLinks = navLinks.filter(link => !link.adminOnly || userProfile?.role === 'administrador')
    ...
    <aside className="w-64 h-screen bg-slate-900 text-slate-300 flex flex-col justify-between shadow-lg">
      ...
      <div>
        <div className="p-4 border-b border-slate-700">
          <h1 className="text-2xl font-bold text-white tracking-tight">FrontStock</h1>
        </div>
